...
export default function ShipmentTimelineItem({
   ...
   return (
      <div className="flex h-20  items-center mb-8 w-full">
         <div className="flex items-center relative">
            <div
               className={`w-6 h-6 rounded-full  flex items-center justify-center z-10
            ${status?.code === "completed" ? "bg-green-500 border-green-500" : "bg-gray-800/50 "}`}
            >
               {Icon}
            </div>
            {!isLast && (
              ...
              ${status?.code === "completed" ? "bg-green-500" : "bg-gray-300/20"}`}
               ></div>
            )}
         </div>
                  ...
                  <div className="flex flex-row gap-2">
                     <h3
                        className={`font-semibold text-xs ${
                           status?.code === "completed" ? "text-green-500" : "text-gray-300"
                        }`}
                     >
                        {status?.name}
                     </h3>
