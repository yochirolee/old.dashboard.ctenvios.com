...
const api = {
   ...
   containers: {
      ...
      updateContainerShipmentsStatus: async (containerId: number, statusId: number, timestamp: Date) => {
   ...
   shipments: {
      ...
      scan: async (hbl: string, statusId: number, timestamp: Date, lat: number | null, loc: number | null) => {
   ...
   stats: {
      ...
      getEmployeesSalesByMonth: async (month: number, year: number): Promise<any[]> => {
         const response = await axiosInstance.get(`${baseUrl}/stats/employees-sales-by-month`, {
            params: {
               month,
               year,
            },
